...
Proof.
  ...
  Case "->". inversion H. subst. 
    SCase "E_IfTrue". rewrite Hb in H5. inversion H5. 
    SCase "E_IfFalse". assumption.
  Case "<-". apply E_IfFalse. rewrite Hb. reflexivity. assumption.
  Qed.
(** [] *)
...
Proof.
  ...
  split;intros H.
  Case "->". inversion H. subst. 
  SCase "beval b=true". apply E_IfFalse. simpl. rewrite H5. reflexivity. assumption.
  SCase "beval b=false". apply E_IfTrue. simpl. rewrite H5. reflexivity. assumption.
  Case "<-". inversion H. subst.
  SCase "beval b=false". apply E_IfFalse. simpl in H5. remember (beval st b) as m.
    destruct m. inversion H5. reflexivity. assumption.
  SCase "beval b=true". apply E_IfTrue. simpl in H5. remember (beval st b) as m.
    destruct m. reflexivity. inversion H5. assumption.
  Qed.
...
(** Write an informal proof of [WHILE_false].
   ...
   _Theorem_: If [b] is equivalent to [BFalse], then [WHILE b DO c END] is equivalent to [SKIP].

   _Proof_: 

     - ([->]) We must show, for all [st] and [st'], that if [WHILE b DO c END/ st || st'] then [SKIP / st || st'].

       Proceed by cases on the rules that could possibly have been
       used to show [WHILE b DO c END / st || st'], namely
       [E_WhileEnd] and [E_WhileLoop].

       - Suppose the final rule rule in the derivation of [WHILE b DO c END / st || st'] was [E_IfTrue].  We then have, by
         the premises of [E_WhileEnd], that [st = st'] and [SKIP/st'||st'].  This is
         exactly what we set out to prove.

       - On the other hand, suppose the final rule in the derivation
         of [WHILE b DO c END/ st || st'] was [E_WhileLoop].
         We then know that [beval st b = true] and [c / st || st''] and [(WHILE b DO c END) / st'' || st'] .

         Recall that [b] is equivalent to [BFalse], i.e. forall [st],
         [beval st b = beval st BFalse].  In particular, this means
         that [beval st b = false], since [beval st BTrue = true].  But
         this is a contradiction, since [E_WhileLoop] requires that
         [beval st b = true].  Thus, the final rule could not have
         been [E_WhileLoop].

     - ([<-]) We must show, for all [st] and [st'], that if [SKIP / st
       || st'] then [(WHILE b1 DO c1 END) / st || st' ].

       Since [b] is equivalent to [BFalse], we know that [beval st b] =
       [beval st BFalse] = [false].  We can apply [E_WhileEnd] to derive [WHILE b DO c END/ st || st].  [](* FILL IN HERE *)
[]
*)
...
The While loops whose guard are equivalent to true never terminates.
*)
(** [] *)
...
Proof. 
  ...
  split;intros H.
  Case "->".
   assert(H': bequiv b BTrue -> ~( (WHILE b DO c END) / st || st' )). apply WHILE_true_nonterm. apply H' in Hb. unfold not in Hb. apply Hb in H. inversion H.
  Case "<-".
    assert(H':~( (WHILE BTrue DO SKIP END) / st || st' )). apply WHILE_true_nonterm. unfold bequiv. reflexivity. unfold not in H'. apply H' in H. inversion H.
  Qed. 
...
Proof.
  ...
  Case "->".
    replace st' with (update st' X (aeval st' e)). inversion H0. subst. apply E_Ass. reflexivity. unfold aequiv in H. rewrite <- H. 
  ...
  Case "<-".
    ...
    unfold aequiv in H. rewrite <- H. simpl. apply functional_extensionality. intros. rewrite update_same. reflexivity. reflexivity.
  Qed.
...
Proof.
  ...
  rewrite (H12). rewrite (H23 st). reflexivity.  Qed.
...
Theorem CSeq_congruence : forall c1 c1' c2 c2',
  ...
  cequiv (c1;c2) (c1';c2').
Proof. 
  unfold cequiv. intros. split;intros Hce.
  inversion Hce. subst. apply E_Seq with st'0. apply H. assumption. apply H0. assumption.
  inversion Hce. subst. apply E_Seq with st'0. apply H. assumption. apply H0. assumption.
  Qed.
(** [] *)
...
Proof.
  ...
  Case "BLe". 
   simpl. remember (fold_constants_aexp a) as a'. remember (fold_constants_aexp a0) as a0'. destruct a'; destruct a0';
   try(rewrite fold_constants_aexp_sound; rewrite fold_constants_aexp_sound with (a:=a0) ; rewrite <- Heqa'; rewrite <- Heqa0'; reflexivity).
   rewrite fold_constants_aexp_sound; rewrite fold_constants_aexp_sound with (a:=a0) ; rewrite <- Heqa'; rewrite <- Heqa0'. simpl.
   remember (ble_nat n n0) as m.  destruct m; try (reflexivity).
  Case "BNot". 
...
Proof. 
  ...
  Case "WHILE".
    ...
    apply WHILE_true. assumption. apply WHILE_false. assumption.
  Qed.
(** [] *)
...
(** Recall the definition [optimize_0plus] from Imp.v:
   ...
   Fixpoint optimize_0plus_aexp (e:aexp): aexp := 
      ...
      | ANum n => 
          ANum n
      | AId X => AId X
      | APlus a1 a2 => match (optimize_0plus_aexp a1, optimize_0plus_aexp a2) with
                                 | (ANum 0, e2) => e2
                                 | (e1, e2) => APlus e1 e2
                                 end
      | AMinus a1 a2 => match (optimize_0plus_aexp a1, optimize_0plus_aexp a2) with
                                    | (e1, e2) => AMinus e1 e2
                                    end
      | AMult a1 a2 => match (optimize_0plus_aexp a1, optimize_0plus_aexp a2) with
                                 | (e1, e2) => AMult e1 e2
                                 end
      end.
  ...
  Proof. unfold atrans_sound. unfold aequiv. intros. 
             aexp_cases (induction a) Case; simpl;
             try reflexivity;
             try (simpl; rewrite IHa1; rewrite IHa2; simpl; reflexivity).
             remember (optimize_0plus_aexp a1) as m1; remember (optimize_0plus_aexp a2) as m2.
             destruct m1; destruct m2; simpl; try( destruct n; rewrite IHa1; rewrite IHa2;  reflexivity); try (rewrite IHa1; rewrite IHa2;  reflexivity).
  Qed.

  Theorem optimize_0plus_bexp_sound:
    btrans_sound optimize_0plus_bexp.
  Proof. unfold btrans_sound. unfold bequiv. intros. 
  ...
  Proof. unfold ctrans_sound. unfold cequiv. intros c.
             com_cases (induction c) Case; simpl.   
             Case "SKIP". apply refl_cequiv.     
             ...
             Case "IFB".
                 ...
                 SCase "Proof of assertion". apply optimize_0plus_bexp_sound.
                 apply CIf_congruence. assumption. assumption. assumption.
             Case "WHILE".    
                 assert (bequiv b (optimize_0plus_bexp b)).
                 SCase "Proof of assertion". apply optimize_0plus_bexp_sound.
                 apply CWhile_congruence. assumption. assumption.
  Qed.
  Definition fold_constants_0plus_optimizer(c:com):com:=
     optimize_0plus_com (fold_constants_com c).
  Theorem optimizer_sound:
  ctrans_sound fold_constants_0plus_optimizer.
  Proof. unfold ctrans_sound. unfold fold_constants_0plus_optimizer. intros c.
    remember (fold_constants_com c) as c1. assert (H: cequiv c c1). rewrite Heqc1. apply fold_constants_com_sound.
    assert (H0: cequiv c1 (optimize_0plus_com c1)).  apply optimize_0plus_com_sound. apply trans_cequiv with c1. assumption. assumption.
  Qed.
(** [] *)
...
Proof.
  ...
  simpl. unfold update. unfold not in H. remember(beq_id i Y) as m. destruct m. apply beq_id_eq in Heqm. apply H in Heqm. inversion Heqm. reflexivity.
...
Definition subst_equiv_property' := forall i1 i2 a1 a2,
  var_not_used_in_aexp i1 a1 -> cequiv (i1 ::= a1; i2 ::= a2)
         (i1 ::= a1; i2 ::= subst_aexp i1 a1 a2).
...
Proof.
  apply E_Seq with empty_state. apply E_Skip. apply E_Havoc. Qed.
(** [] *)
...
Proof. left. unfold cequiv. unfold pXY. unfold pYX. intros. split.
  Case "->". intros Hce. inversion Hce. inversion H1. subst. inversion H4. subst.
    assert (HXY: update (update st X n) Y n0 = update(update st Y n0) X n). 
      SCase "Proof of assertion". apply functional_extensionality. intros x. apply update_permute; reflexivity.
    rewrite HXY. apply E_Seq with (update st Y n0). apply E_Havoc. apply E_Havoc.
  Case "<-". intros Hce. inversion Hce. inversion H1. subst. inversion H4. subst.
      assert (HXY: update (update st Y n) X n0 = update(update st X n0) Y n). 
        SCase "Proof of assertion". apply functional_extensionality. intros x. apply update_permute; reflexivity.
      rewrite HXY. apply E_Seq with (update st X n0). apply E_Havoc. apply E_Havoc.
  Qed.
...
Proof. 
  right. (* Prove they are not equivalent. *)
  unfold not, cequiv, iff, ptwice, pcopy.
  remember (update (update empty_state X 1) Y 2) as st12. intros.
  remember (H empty_state st12) as Ha. inversion Ha.
  assert ((HAVOC X; HAVOC Y) / empty_state || st12) as Hst12. subst st12.
    apply E_Seq with (update empty_state X 1). apply E_Havoc. apply E_Havoc.
  remember (H0 Hst12) as Hst0. inversion Hst0. subst. 
  inversion H4. subst. clear H4.
  inversion H7. subst. clear H7.
  simpl in H6. rewrite update_eq in H6. clear H. clear H0. clear H1. clear Hst12.
  remember (update (update empty_state X n) Y n) as f.
  assert (f X = f Y).
   subst f. reflexivity.
  rewrite H6 in H. simpl in H. inversion H.
  Qed. 
(** [] *)
