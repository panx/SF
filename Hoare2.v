...
Proof.
  ...
  Case "consistency within the loop".
    eapply hoare_consequence_pre. apply hoare_asgn. unfold assn_sub, assert_implies, bassn, update. intros. 
    SearchAbout beq_id. rewrite <- beq_id_refl. simpl. inversion H. rewrite <- H0. apply parity_ge_2. 
    simpl in H1. destruct (st X). inversion H1. destruct n. inversion H1. omega.
  Case "precondition implies invariant".
    unfold assert_implies. intros st H. rewrite H. reflexivity.
  Case "invariant implies postcondition".
    intros st [Hinvar Hguard]. rewrite <- Hinvar. symmetry. apply parity_lt_2. 
    unfold not. unfold not, bassn in Hguard. intros. apply Hguard. simpl.
    destruct (st X). inversion H. destruct (n). inversion H. inversion H1. reflexivity.
  Qed.
(** [] *)
...
(** The following program computes the square root of [X]
 ...
 (8)  {{ Z*Z<=m /\ m<(Z+1)*(Z+1) }}
    ...
    under the assumption that [X=m].  So we now try [X=m /\ Z*Z <= m]
...
Proof.
  ...
  Case "left".
    eapply hoare_consequence_pre.
      apply hoare_asgn. 
      unfold assert_implies, assn_sub, update. intros. rewrite <- beq_id_refl. simpl. omega.
  Case "right".
     unfold hoare_triple. unfold assert_implies. intros.
     remember (update st X (aeval st (APlus (AId Y) (ANum 1)))) as st'.
     assert (H1: (X ::= APlus (AId Y) (ANum 1)) / st || st' -> P' st -> st' X <= 5). apply H. rewrite Heqst' in H1. simpl in H1. unfold update in H1.
       rewrite <- beq_id_refl in H1. apply le_S_n. assert (S(st Y) = st Y + 1). omega. rewrite H2. apply H1. apply E_Ass. reflexivity. assumption.
  Qed.
...
Proof.
  ...
  Case "right". 
    ...
    apply H. unfold assn_sub. rewrite <- Heqst'. 
      apply H1. rewrite Heqst'. apply E_Ass. reflexivity. assumption.
  Qed.
(** [] *)
...
Lemma hoare_havoc_weakest : forall (P Q : Assertion) (X : id),
  ...
  P ->> havoc_pre X Q.
Proof.
  unfold hoare_triple, havoc_pre, assert_implies. intros.
  remember (update st X n) as st'. assert ((HAVOC X) / st || st' -> P st -> Q st'). apply H.
  apply H1. rewrite Heqst'.  apply E_Havoc. assumption.
  Qed.
End Himp2.
(** [] *)
...
(** Write a corresponding formal decorated program (parametrized by [m])
...
(* $Date: 2013-03-14 17:57:12 -0400 (Thu, 14 Mar 2013) $ *)
