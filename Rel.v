...
Proof.
  ...
  intros n m o Hnm Hmo.
  induction o as [| o'].
    apply ex_falso_quodlibet. inversion Hmo.
    inversion Hmo. subst. apply le_S. apply Hnm.
...
Theorem le_S_n : forall n m,
  (S n <= S m) -> (n <= m).
Proof.
  intros n m H. inversion H. apply le_n. apply le_Sn_le. assumption. Qed.
(** [] *)
...
Proof.
  ...
  inversion H. apply IHn. apply le_S_n. apply H.
  Qed. 
(** [] *)
...
Proof.
  ...
  assert (0 <= 1 -> 1 <=0). apply H.
  assert (1<=0).  apply H0. apply le_S. apply le_n.
  inversion H1.
  Qed.
(** [] *)
...
Proof.
  ...
  induction H; induction H0; try subst; try reflexivity.
  assert (m <= S m). apply le_S. apply le_n.
  assert (S m0 <= S m). apply le_trans with m; assumption. 
  assert (S m0 <= m0). apply le_trans with (S m); assumption.
  apply le_Sn_n in H3. inversion H3.
  Qed.
...
Theorem le_step : forall n m p,
  ...
  n <= p.
Proof. 
  intros. destruct m. inversion H. unfold lt in H. 
  apply le_S_n in H. apply le_S_n in H0. apply le_trans with m; assumption.
  Qed.
(** [] *)
...
Proof.
  ...
  induction H. 
    assumption.
    apply rsc_step with y. assumption. apply IHrefl_step_closure. assumption.
  Qed.
(** [] *)
  ...
  Case "->".
    intros. induction H.
      ...
      SCase "rt_refl".
        apply rsc_refl.
      SCase "rt_trans".
        apply rsc_trans with y; try assumption.
  Case "<-".
    intros. induction H.
    SCase "rsc_refl". 
      apply rt_refl.
    SCase "rsc_step".
      apply rt_trans with y. apply rt_step. assumption.
      assumption.
  Qed.
(** [] *)
