...
Proof.
   ...
   split. unfold normal_form. unfold not. intros. destruct H. inversion H. subst. inversion H1.
...
Proof.
  ...
  induction H; unfold normal_form; unfold not; intros.
    ...
    destruct H0. inversion H0. subst. unfold normal_form in IHnvalue. unfold not in IHnvalue. apply IHnvalue. exists t1'. assumption.
  Qed.
    ...
    SCase "t1 is value". inversion H;subst.
      ...
      SSCase "t1 is a nvalue". induction H0.
        ...
        SSSCase "t1 is tssuc t".
          left. auto.
    SCase "t1 can take a step".
      right. destruct H. exists (tsucc x). apply ST_Succ. assumption.
   Case "T_Pred".
     inversion IHHT.
...
(** _Proof_: By induction on a derivation of [|- t \in T].
     ...
     - If the last rule in the derivation is T_Succ, then [t = |- tsucc t1 \in TNat ], with [|- t1 \in Nat].
        By the IH, either [t1] is a value or else [t1] can step to some [t1'].
       
         - If [t1] is a value, then it is either an [nvalue] or a [bvalue]. But it cannot be an [bvalue],
           because we know [|- t1 \in Nat] and there are no rules assigning type [Nat] to any term 
           that could be a [bvalue]. So [t1] is a [nvalue], --ie., it is either [tzero] or [tssuc t'].
           If [t1=tzero], then by nv_succ, [tsucc t1] is a value.
           If [t1 = tsucc t'], then by nv_succ [tsucc t1] is a value.
        
        - If [t1] could take a step, then by [ST_Succ], so can [tsucc t1].
...
(** _Proof_: By induction on a derivation of [|- t \in T].
     ...
     - If the last rule in the derivation is [T_Succ], then [t = tsucc t1] and 
       [|- t1 \in Nat].         
       Inspecting the rules for the small-step reduction relation and
        remembering that [t] has the form [tssuc t1], we see the only one that
       could have been used to prove [t ==> t'] is [ST_Succ]. Then we have 
       [t1 ==> t1'] and [t' = tsucc t1'].
       To prove [|- t' \in Nat], by [T_succ], we can prove [|- t1' \in Nat]. This could 
       be proved by induction hypothesis.
...
Proof with eauto.
  ...
  step_cases (induction H0) Case; intros.
  inversion H; subst; assumption. 
  inversion H; subst; assumption.
  inversion H; subst. apply IHstep in H4. apply T_If; try assumption.
  inversion H; subst. apply IHstep in H2. apply T_Succ. assumption.
  inversion H; subst. assumption.
  inversion H0; subst. inversion H2; subst. assumption.
  inversion H; subst. apply T_Pred. apply IHstep. assumption.
  inversion H; subst. apply T_True.
  inversion H0; subst. apply T_False.
  inversion H; subst. apply IHstep in H2. apply T_Iszero. assumption.
  Qed.
(** [] *)
...
(** Suppose, instead, that we add this new rule to the [step] relation: 
   ...
   Deterministic becomes false.
   counter example: [tif ttrue ttrue tfalse] would now have two next step.
[]
*)
