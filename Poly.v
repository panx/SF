...
Proof. intros X v s. induction s as [|t s']. 
  reflexivity. 
...
Theorem rev_involutive : forall X : Type, forall l : list X,
  rev (rev l) = l.
Proof. intros X l. induction l as [|t l'].
 reflexivity. simpl. rewrite rev_snoc. rewrite IHl'. reflexivity. Qed.

Theorem snoc_append : forall X: Type, forall l:list X, forall n:X,
  snoc l n = l ++ [n].
Proof. intros X l n.
...
Proof.
  intros X l1 l2 v. induction l2 as [|t l']. 
  simpl. replace (l1 ++ []) with (l1). rewrite snoc_append. reflexivity.
  induction l1 as [|t l1']. reflexivity. simpl.  rewrite <- IHl1'. reflexivity.
  simpl. rewrite snoc_append. rewrite snoc_append. rewrite app_ass. reflexivity.
Qed. 
(** [] *)
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => andb (evenb n) (ble_nat (S(S(S(S(S(S(S O))))))) n)) l.
...
Definition partition {X : Type} (test : X -> bool) (l : list X)
                     : list X * list X :=(filter test l, filter (fun f => negb (test f)) l).
...
Proof.
  intros X Y f l. induction l as [|t l']. 
  reflexivity. simpl.  rewrite map_snoc. rewrite IHl'. reflexivity.
Qed.
(** [] *)
...
Proof. intros X l. unfold fold_length. induction l. 
  reflexivity. simpl. rewrite IHl. reflexivity. Qed.
(** [] *)
...
Definition fold_map {X Y:Type} (f : X -> Y) (l : list X) : list Y :=
  fold (fun  x  sum=> f x :: sum  ) l [].

(** Write down a theorem in Coq stating that [fold_map] is correct,
    and prove it. *)
Theorem fold_map_correct: forall {X Y:Type} (f: X->Y) (l: list X),
  fold_map f l = map f l.
Proof. intros X Y f l.
unfold fold_map.  induction l.  reflexivity. simpl. rewrite IHl. reflexivity. Qed.
