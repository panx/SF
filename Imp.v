...
*)
Fixpoint optimize_1mult (e:aexp) : aexp := 
  match e with
  | ANum n => 
      ANum n
  | APlus e1 e2 => 
...
Proof.
  intros. destruct i1. destruct i2. unfold beq_id in H. apply beq_nat_eq in H. subst. reflexivity. Qed.
(** [] *)
...
Proof.
  intros. destruct i1. destruct i2. unfold beq_id in H. apply beq_nat_false in H. unfold not in H. unfold not. intros. apply H. inversion H0. reflexivity. Qed. 
(** [] *)
...
Proof.
  intros. unfold not in H. destruct i1. destruct i2. unfold beq_id. apply beq_nat_false_iff. unfold not. intros. apply H. rewrite H0. reflexivity. Qed.
(** [] *)
...
Proof.
  intros. destruct i1. destruct i2. unfold beq_id. generalize dependent n. induction n0.
  destruct n. reflexivity. reflexivity.
  destruct n. reflexivity. simpl. apply IHn0.
  Qed.
(** [] *)
...
Proof.
  intros. unfold update. unfold beq_id. simpl. reflexivity. Qed.
(** [] *)
...
Proof.
  intros. unfold update. destruct (beq_id k2 k1). reflexivity. reflexivity. Qed.
(** [] *)
...
Proof.
  intros. unfold update. remember (beq_id k1 k2) as m. destruct m. apply beq_id_eq in Heqm. subst. reflexivity. reflexivity. Qed.
(** [] *)  
...
Proof.
  intros. unfold update. remember (beq_id k1 k3) as m. destruct m. apply beq_id_eq in Heqm. subst. rewrite H. reflexivity. reflexivity. Qed.
(** [] *)
...
Theorem XTimesYinZ_comm: forall st st',
  st X = 0 -> XtimesYinZ / st || st' -> st' Z =0.
Proof. intros st st' H0 H1. unfold XtimesYinZ in H1. inversion H1. unfold update. rewrite <- beq_id_refl. rewrite <- H5. simpl. rewrite H0. simpl. reflexivity. Qed.
...
Proof.
  ...
  (* Proceed by induction on the assumed derivation showing that
     ...
     [inversion]). *)
  induction contra; try (inversion Heqloopdef).  subst. inversion H. subst. apply IHcontra2. reflexivity. Qed.
...
Inductive no_whilesR: com -> Prop := 
   ...
   | N_ASS: forall a X, no_whilesR (X::=a)
...
Proof.
  intros c. split. intros h. induction c.
  apply N_SKIP. apply N_ASS. inversion h. apply andb_true_iff in H0. inversion H0. apply N_SEQ. apply IHc1. apply H. apply IHc2. apply H1.
  apply N_IFB; inversion h; apply andb_true_iff in H0; inversion H0. apply IHc1 in H. apply H. apply IHc2 in H1. apply H1. 
  inversion h. 
  intros. induction c; try (simpl). 
  reflexivity.
  reflexivity.
  apply andb_true_iff. inversion H. split. apply IHc1 in H2. apply H2. apply IHc2 in H3. apply H3.
  apply andb_true_iff. inversion H. split. apply IHc1 in H2. apply H2. apply IHc2 in H4. apply H4.
  inversion H.
  Qed.
...
Proof. 
  ...
  intros st. exists (update st X0 (aeval st a)). apply E_Ass. reflexivity.
  intros st. 
    assert (H1: exists st0: state, c1/st||st0). apply IHno_whilesR1. inversion H1.
    assert(H3: exists st': state, c2/x||st'). apply IHno_whilesR2. inversion H3. 
    exists x0. apply E_Seq with x. apply H2. apply H4.
  intros st. 
    assert (H1: exists st0: state, ct/st||st0). apply IHno_whilesR1. inversion H1. 
    assert (H3: exists st1: state, cf/st||st1). apply IHno_whilesR2. inversion H3.
    remember (beval st b) as eval_b. destruct eval_b. 
    exists x. apply E_IfTrue. rewrite Heqeval_b. reflexivity.  apply H2.
    exists x0. apply E_IfFalse. rewrite Heqeval_b. reflexivity. apply H4.
  Qed.
(** [] *) 
Theorem no_whilesR_terminate:
  forall c, no_whiles c=true -> forall st, exists st', c / st || st'.
Proof. 
  intros c H. induction c.
  intros st. exists st. apply E_Skip.
  intros st. exists (update st i (aeval st a)). apply E_Ass. reflexivity.
  intros st. 
    assert (H1: exists st0: state, c1/st||st0). apply IHc1. simpl in H. apply andb_true_iff in H. inversion H. apply H0. inversion H1.
    assert (H2: exists st1: state, c2/x||st1). apply IHc2. simpl in H. apply andb_true_iff in H. inversion H. apply H3. inversion H2.
    exists x0. apply E_Seq with x. apply H0. apply H3.
  intros st.
    assert (H1: exists st0: state, c1/st||st0).  apply IHc1. simpl in H. apply andb_true_iff in H. inversion H. apply H0.
    assert (H2: exists st1: state, c2/st||st1). apply IHc2. simpl in H. apply andb_true_iff in H. inversion H. apply H2.
    remember (beval st b) as b_eval. destruct b_eval.  
    inversion H1. exists x. apply E_IfTrue. rewrite Heqb_eval. reflexivity. apply H0.
    inversion H2. exists x. apply E_IfFalse.  rewrite Heqb_eval. reflexivity. apply H0.
  inversion H.
  Qed.
...
Theorem short_circuit_beval_correct: forall st b,
  beval st b = beval' st b.
Proof. 
  intros. destruct b; reflexivity. Qed.
...
Fixpoint s_execute (st : state) (stack : list nat) 
  ...
  | h::t => match h with
               ...
               | (SLoad id) => s_execute st ((aeval st (AId id))::stack) t
               | SPlus => match stack with
                               | [] => []
                               | n1::t' => match t' with
                                             | [] => []
                                             | n2::t'' =>  s_execute st ((n1+n2)::t'') t
                                             end
                               end
