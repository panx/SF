...
Proof.
  intros n h. destruct n as [|n']. reflexivity. inversion h. Qed.
...
Proof.
  ...
  Case "n=0". intros m H. induction m as [|m'].
    SCase "m=0". reflexivity. 
    SCase "m=S m'". simpl in H. inversion H.
  Case "n=S n'". intros m H. destruct m as [|m'].
    SCase "m=0". simpl in H. inversion H. 
    SCase "m=S m'". inversion H.  rewrite <- plus_n_Sm in H1. rewrite <- plus_n_Sm in H1. inversion H1. apply IHn' in H2. rewrite H2. reflexivity. Qed.
...
Proof.
  intros n. induction n as [|n']. 
  Case "n=0". intros m eq. destruct m as [|m']. 
    SCase "m=0". reflexivity. 
    SCase "m=S m'". inversion eq.
  Case "n=Sn'". intros m eq. destruct m as [|m'].
    SCase "m=0". inversion eq. 
    SCase "m= S m'". apply IHn' in eq. rewrite eq. reflexivity.
  Qed.
...
Theorem index_after_last: forall (n : nat) (X : Type) (l : list X),
     ...
     index (S n) l = None.
Proof.
  intros n X l eq. generalize dependent n. induction l as [|h t]. 
  Case "l=[]". intros n eq. simpl. reflexivity.
  Case "l=h::t". intros n eq. simpl in eq. simpl. rewrite <- eq. apply IHt. reflexivity.
  Qed.
(** [] *)
...
Theorem length_snoc''' : forall (n : nat) (X : Type) 
     ...
     length (snoc l v) = S n. 
Proof.
  intros n X v l. generalize dependent n. induction l as [|h t].
  Case "l=[]". intros n eq. simpl. simpl in eq. rewrite <- eq. reflexivity.
  Case "l=h::t". intros m eq. destruct m.
    SCase "m=0". simpl in eq. inversion eq.
    SCase "m=S m'". simpl in eq. inversion eq. simpl. apply IHt in H0. rewrite H0. rewrite eq. reflexivity.
  Qed.
(** [] *)
...
Theorem app_length_cons : forall (X : Type) (l1 l2 : list X) 
     ...
     S (length (l1 ++ l2)) = n.
Proof.
  intros X l1 l2 x n. generalize dependent l2. generalize dependent n. induction l1 as [|h t].
  Case "l1=[]". intros n l2 eq. simpl. simpl in eq. apply eq.
  Case "l1= h::t ". intros m l2 eq. destruct m as [|m'].
    SCase "m=0". inversion eq.
    SCase "m=S m'". simpl in eq. inversion eq. simpl. apply IHt in H0. rewrite H0. rewrite eq. reflexivity.
  Qed.
(** [] *)
...
Proof.
  ...
  Case "l=[]". intros n eq. destruct n as [|n']. 
    SCase "n = 0". simpl. reflexivity.
    SCase "n = S n'". simpl in eq. inversion eq.
  Case "l = h::t". intros m eq. induction m as [|m']. 
    SCase "m=0". inversion eq.
    SCase "m=S m'".  simpl in IHm'. simpl in eq. inversion eq. rewrite H0. apply IHt in H0.  Admitted.
(** [] *)
...
Theorem combine_split : forall X Y (l : list (X * Y)) l1 l2,
  split l = (l1, l2) ->
  combine l1 l2 = l.
Proof. intros X Y l l1 l2. Admitted.*)
(** [] *)
...
Proof. intros f b. destruct b.
  Case "b=true". 
  remember (f true) as e0.
  remember (f false) as e1. 
    destruct (e0). 
      destruct (e1). 
        rewrite <- Heqe0. rewrite <- Heqe0. reflexivity.
        rewrite <- Heqe0. rewrite <- Heqe0. reflexivity.
     destruct e1. 
        rewrite <- Heqe1. rewrite Heqe0. reflexivity.
        rewrite <- Heqe1. rewrite <- Heqe1. reflexivity.
  Case "b=false".
  remember (f true) as e0.
  remember (f false) as e1.
    destruct (e0). 
      destruct (e1). 
        ...
        rewrite <- Heqe1. rewrite <- Heqe1. reflexivity.
      destruct (e1).
        rewrite <- Heqe0. rewrite Heqe1. reflexivity.
        rewrite <- Heqe1. rewrite <- Heqe1. reflexivity.
  Qed.
(** [] *)
...
Proof.
  intros X x1 k1 k2 f eq. unfold override. remember (beq_nat k1 k2) as e0. destruct e0. 
    apply beq_nat_eq in Heqe0. rewrite <- Heqe0. symmetry in eq. apply eq.
    reflexivity.
Qed.
(** [] *)
...
Proof.
  intros X x1 x2 k1 k2 k3 f eq. unfold override. remember (beq_nat k1 k3) as e0.
  destruct (e0).
  Case "k1=k3". apply beq_nat_eq in Heqe0. rewrite Heqe0 in eq. symmetry in eq. rewrite eq. reflexivity.
...
Example trans_eq_exercise : forall (n m o p : nat),
     ...
     (n + p) = (minustwo o). 
Proof.
  intros n m o p eq1 eq2. symmetry in eq1. symmetry in eq2. symmetry. rewrite eq2 in eq1. apply eq1. Qed.
(** [] *)
...
Proof.
  intros n m. remember (beq_nat n m) as e.
  induction e. apply beq_nat_eq in Heqe. rewrite Heqe. apply beq_nat_refl.
  remember (beq_nat m n) as e1. destruct e1. apply beq_nat_eq in Heqe1. rewrite Heqe1 in Heqe. rewrite <- beq_nat_refl in Heqe. inversion Heqe.
  reflexivity.
  Qed.
