...
(* $Date: 2013-02-20 17:26:47 -0500 (Wed, 20 Feb 2013) $ *)
...
(** The evaluators we have seen so far (e.g., the ones for [aexp]s,
    ...
    lists that we introduced in the [HoareList] chapter.  We chose to
    ...
    do this: we'd just have to use the formulations of [aeval] and
    [beval] as inductive propositions (rather than Fixpoints), so that
    we can make them partial functions instead of total ones.
...
Proof.
  ...
  generalize dependent y2.
  step_cases (induction H) Case.
    Case "ST_PlusConstConst". intros. step_cases (inversion H0) SCase.
      SCase "ST_PlusConstConst". reflexivity.
      SCase "ST_Plus1". inversion H3.
      SCase "ST_Plus2". inversion H4.
    Case "ST_Plus1". intros. step_cases (inversion H0) SCase.
      SCase "ST_PlusConstConst". rewrite <- H2 in H. inversion H.
      SCase "ST_Plus1". apply IHstep in H4. subst. reflexivity.
      SCase "ST_Plus2". inversion H3. rewrite <- H6 in H. inversion H.
    Case "ST_Plus2". intros. step_cases (inversion H1) SCase.
      SCase "ST_PlusConstConst". rewrite <- H4 in H0. inversion H0.
      SCase "ST_Plus1". inversion H. rewrite <- H6 in H5. inversion H5.
      SCase "ST_Plus2". apply IHstep in H6. subst. reflexivity.
  Qed.
(** [] *)
...
Proof.
  intros. exists (P (P (C 1)(C 1)) (C 1)).
  split. 
    apply v_funny. 
    unfold not. unfold normal_form. unfold not. intros.
    apply H. exists (P(C (1+1))(C 1)). apply ST_Plus1. apply ST_PlusConstConst. 
  Qed.
(** [] *)
...
Lemma value_not_same_as_normal_form :
  exists t, value t /\ ~ normal_form step t.
Proof.
  intros. exists (C 1). split.  
  apply v_const.
  unfold normal_form, not. intros. apply H. exists (P (C 1)(C 0)). apply ST_Funny.
  Qed.
(** [] *)
...
Proof.
  ...
  unfold normal_form. unfold not. intros. inversion H. inversion H0. subst. inversion H4.
  Qed.
(** [] *)
...
Example not_bool_step_prop1: 
  ~(bool_step_prop1).
Proof. 
  unfold not, bool_step_prop1. intros.
  inversion H.
  Qed.
...
Example not_bool_step_prop2: ~ bool_step_prop2.
Proof.
  unfold bool_step_prop2, not. 
  intros. inversion H.
  Qed.
...
Proof.  
  intros. induction t.
    ...
    right. 
      ...
      inversion H. exists (tif x t2 t3). apply ST_If. assumption.
  Qed.
...
Proof.
  ...
  generalize dependent y2.
  induction H; intros.
    inversion H0. subst. reflexivity. subst. inversion H4.
    inversion H0. subst. reflexivity. subst. inversion H4.
    inversion H0. subst. inversion H. inversion H2. subst. inversion H.
...
Example bool_step_prop4_holds : 
  bool_step_prop4.
Proof.
  unfold bool_step_prop4.  apply ST_IfThen. Qed.
(** [] *)
...
(* Answer: no longer deterministic. Because when it is tif t1 t2 t2, it could have two results.*)
    Example not_deterministic_step: ~ (deterministic step).
    Proof. unfold not, deterministic. intros.
      assert (tif ttrue ttrue ttrue = ttrue).
      apply H with (x := tif (tif ttrue ttrue ttrue) ttrue ttrue).
...
*)
  ...
  (*Answer: yes. Because it does give more chance or reduce any change to be evaluated to another term.*)
  Theorem strong_progress : forall t,
  value t \/ (exists t', t ==> t').
...
(**
  ...
  (* Yes. If we take away any of the first three constructors, it would fail to make strong progress to fail.*)
*)
(** [] *)
...
Proof.
  ...
  eapply multi_step. apply ST_Plus2. apply v_const. apply ST_PlusConstConst. apply multi_refl.
  Qed.
(** [] *)
...
Proof.
  ...
  apply nf_same_as_value in P12. inversion P12. subst.
  inversion P21. reflexivity. inversion H.

  apply IHP11; try assumption.
  inversion P21;subst. apply nf_same_as_value in P22. inversion P22. subst. inversion H. 
    assert (y0 =y). apply step_deterministic with x; assumption. subst. apply H1.
  Qed.
...
Proof.
  ...
  eapply multi_trans.
  apply multistep_congr_2. constructor. eassumption.
...
(** Write a detailed informal version of the proof of [eval__multistep].
  ...
  Theorem: if t is evaluated to constant n, then we can get from
    [t] to [C n] using the [step] relation zero or more times.
  Proof:
    Proceed by cases on the rules that could possibly have been
    used to show t || n, namely [E_Const] and [E_Plus].
    
    - Suppose the final rule in the derivation of [t || n] was [E_Const].  We then have, by
      the premises of [E_Const], that [t = C n]. Then what we wanted to prove is [C n ==>* C n].
      This could be proved by the rule multi_refl.
   
   - Suppose the final rule in the derivation of [t || n]  was [E_Plus].  We then have, by 
      the premises of [E_Plus], that [t = P t1 t2], [ t1 || n1 ] and [ t2 || n2 ]. By induction, we also have that
      [t1 ==>* n1] and [t2 ==>* n2].

      By the lemma multi_trans, we can prove [P t1 t2 ==>* C (n1 + n2)] by showing
      [P t1 t2 ==>* P (C n1) t2], [P (C n1) t2 ==>* P (C n1 + n2)]. 
      
      [P t1 t2 ==>* P (C n1) t2] could be proved by lemma multistep_congr_1 with the premise [t1 ==>* C n1], 
      which is already in the context.

      By the lemma multi_trans, we can prove [P (C n1) t2 ==>* C (n1 + n2)] by showing
      [P (C n1) t2 ==>* P (C n1) (C n2)], [P (C n1) (C n2) ==>* P (C n1 + n2)].
      
      [P (C n1) t2 ==>* P (C n1) (C n2)] could be proved by lemma multistep_congr_2 with the premise [t2 ==>* C n2],
      which is already in the context.
      [P (C n1) (C n2) ==>* P (C n1 + n2)] could be constructed by constructor ST_PlusConstConst.
...
Proof.
  ...
  induction Hs; intros.
    inversion H. subst. apply E_Plus. apply E_Const. apply E_Const.
...
Proof.
  intros. inversion H. unfold step_normal_form in H1. rewrite nf_same_as_value in H1. inversion H1. subst.
  induction H0.
    inversion H1. exists n0. split. reflexivity. apply E_Const.
    inversion H1. exists n0. split. reflexivity. apply step__eval with y. assumption. destruct IHmulti.
      unfold normal_form_of. split. assumption. apply nf_same_as_value. assumption.
      assumption. destruct H4. subst. inversion H3. subst. assumption.
  Qed.
(** [] *)
...
Proof. 
  unfold deterministic. intros. generalize dependent y2.
    ...
    Case "ST_PlusConstConst".
      inversion H0; subst; try(reflexivity).  inversion H3. inversion H4.
    Case "ST_Plus1".
      inversion H0; subst. inversion H. subst. apply IHstep in H4. subst. reflexivity.
      inversion H3. subst. inversion H. subst. inversion H. subst. inversion H.
    Case "ST_Plus2".
      inversion H1. subst. inversion H0. subst. inversion H. subst. inversion H5.
      subst. inversion H5. subst. inversion H5. apply IHstep in H6. subst. reflexivity.
    Case "ST_IfTrue".
      inversion H0. subst. reflexivity. inversion H4.
    Case "ST_IfFalse".
      inversion H0. subst. reflexivity. inversion H4.
    Case "ST_If".
      inversion H0; subst. inversion H. inversion H. apply IHstep in H5. subst. reflexivity.
  Qed.

Theorem strong_progress : exists t,
  ~ (value t) /\ ~(exists t', t ==> t').
Proof.
  exists (P ttrue ttrue). split; unfold not; intros.
  inversion H. destruct H. inversion H. subst. inversion H3. subst. inversion H4.
Qed.
(** [] *)
...
Proof.
  ...
  eapply multi_step. apply CS_Par2. apply CS_IfStep.
    apply BS_Eq1. apply AS_Id.
  eapply multi_step. apply CS_Par2. inversion H. rewrite H1.
    apply CS_IfStep. apply BS_Eq. simpl.
  eapply multi_step.  apply CS_Par2. apply CS_IfTrue.
  eapply multi_step. apply CS_Par2. apply CS_SeqStep.
    apply CS_AssStep. apply AS_Plus1. apply AS_Id.
  eapply multi_step. apply CS_Par2. inversion H. rewrite H0. 
