...
Proof.
  ...
  Case "->".
   generalize dependent t'.
    ...
    simpl in H. remember (beq_id x0 i) as m.
      destruct m. subst. 
          apply beq_id_eq in Heqm. subst. apply s_var1.
          subst. apply s_var2. symmetry. assumption.
    destruct t'; try (inversion H).
        apply s_tapp. apply IHt1. reflexivity. apply IHt2. reflexivity.
    simpl in H. remember (beq_id x0 i) as m.
      destruct m. subst. apply s_tabs1. symmetry. apply Heqm.
      subst. apply s_tabs2. symmetry. apply Heqm. apply IHt. reflexivity.
     simpl in H. subst. apply s_ttrue.
     simpl in H. subst. apply s_tfalse.
     simpl in H. subst. apply s_tif. apply IHt1. reflexivity. apply IHt2. reflexivity. apply IHt3. reflexivity.
   Case "<-".
   induction H; try (simpl; rewrite H; reflexivity).
   simpl. rewrite <- beq_id_refl. reflexivity.
   simpl. rewrite H. subst. reflexivity.
...
Example typing_example_2_full :
  empty |-
    ...
    (TArrow TBool (TArrow (TArrow TBool TBool) TBool)).
Proof.
  apply T_Abs. apply T_Abs. apply T_App with TBool.
  apply T_Var. unfold extend. rewrite <- beq_id_refl. reflexivity.
  apply T_App with TBool.  apply T_Var. unfold extend. rewrite <- beq_id_refl. reflexivity.
     apply T_Var. unfold extend. simpl. reflexivity.
  Qed.
(** [] *)
...
Proof with auto.
  ...
  apply T_App with TBool.
    apply T_Var. unfold extend. simpl. reflexivity.
    apply T_App with TBool. apply T_Var. unfold extend. reflexivity.
                                            apply T_Var. unfold extend. reflexivity.
Qed.
(** [] *)
