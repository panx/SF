...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  | nil => true
  | h :: t => if blt_nat 0 (count h s2) then subset t (remove_one h s2) else false
  end.
...
Example beq_nat_mm: forall m: nat,
  beq_nat m m = true.
...
Theorem add_count : forall b : bag, forall n : nat,
  count n (add n b ) = 1 + count n b.
Proof.
  intros b n.
  simpl. induction b.
  rewrite -> beq_nat_mm. reflexivity.
  rewrite -> beq_nat_mm. reflexivity.
  Qed. 
(** [] *)
...
Example length_repeat: forall n m: nat, 
  length (repeat n m) = m.
Proof. intros n m.
induction m as [|m'].
reflexivity.
...
Theorem foo2 :  forall n m:nat, forall l: natlist,
repeat n m = l -> length l = m.
Proof. intros n m l H.
rewrite <- H.
rewrite length_repeat. reflexivity.
Qed.
...
Example length_snoc_2: forall l : natlist, forall n : nat,
  length (snoc l n)= S (length l).
Proof. intros l n.
induction l as [|l'].
simpl. reflexivity.
...
Theorem foo3 :   forall l: natlist, forall n m:nat,
  length l = m -> length (snoc l n) = S m.
Proof. intros l n m H.
rewrite <- H.  rewrite length_snoc_2. reflexivity.
Qed.
...
Example length_snoc_3: forall l : natlist, forall n : nat,
  blt_nat 0 (length (snoc l n))=true.
Proof. intros l n.
induction l as [|l'].
reflexivity. 
...
Theorem foo4 :  forall n :nat, forall l1 l2: natlist,
  snoc l1 n = l2 -> (blt_nat 0 (length l2)) = true.
Proof. intros n l1 l2 H. rewrite <- H. rewrite length_snoc_3. reflexivity.
Qed.
...
Example rev_snoc : forall (l:natlist) (n:nat),
  rev (snoc l n) = n::(rev l).
Proof. intros. induction l.
reflexivity. simpl. rewrite IHl. simpl. reflexivity. Qed.
...
Proof. intros l1 l2 l3 l4.
  simpl. rewrite app_ass. rewrite app_ass. reflexivity. Qed.
...
Example snoc_ass_1 : forall (l1 l2 : natlist) (n:nat),
...
Proof. intros l1 l2.
  ...
  simpl. rewrite IHt. rewrite snoc_ass_1. reflexivity.
  Qed.
...
(** Design exercise: 
     - Write down a non-trivial theorem involving [cons]
       ([::]), [snoc], and [append] ([++]).  
     - Prove it. *) 
Theorem list_design : forall h : nat, forall t : natlist,
  app t (h::[]) = snoc t h.
Proof. intros h t.
  induction t.
  reflexivity.
...
Proof. intros l.
  induction l as [|h t].
  simpl. reflexivity.
  simpl. rewrite beq_nat_mm. rewrite IHt. reflexivity. 
Qed.
(** [] *)
...
Proof. intros d k v.
simpl. rewrite beq_nat_mm. reflexivity. Qed.
(** [] *)
