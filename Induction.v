...
(* Destruct tactic is mainly focused on using separate cases to analyze the goal and then prove case by case.*)
(* Even though induction tactic also have separate cases. it mainly uses the induction way to prove the theorem.*)
...
Proof.
  ...
  Case "n=S n'".
    simpl. rewrite IHn'. rewrite mSn. reflexivity.
  Qed.
...
Theorem plus_swap2: forall n m p q: nat,
  n+m+(p+q)= n+p+(m+q).
Proof. 
  intros n m p q. induction n as [|n'].
...
Proof.
  ...
  simpl. rewrite mSn. rewrite mSn. rewrite mSn. rewrite IHp'. rewrite plus_swap2. reflexivity.
Qed. 

Theorem Smn : forall m n: nat,
  S m * n = n * S m.
Proof. 
  intros m n. simpl. rewrite mSn. rewrite mult_comm. reflexivity. Qed.



Theorem mult_comm2 : forall n m p: nat,
  p * m * n = n * m * p.
Proof. 
  intros n m p. induction m as [|m'].
  rewrite mult_0_r. rewrite mult_0_r. reflexivity.
  rewrite mSn. rewrite mSn. rewrite mult_plus_distr_r. rewrite mult_plus_distr_r. rewrite IHm'. rewrite mult_comm. reflexivity.
  Qed.
...
Proof.
  intros n m p. induction m as [|m'].
  simpl. rewrite mult_0_r. reflexivity.
  rewrite Smn. rewrite mSn. rewrite mSn. rewrite mult_comm. rewrite mult_plus_distr_r. rewrite mult_plus_distr_r.  rewrite mult_comm2. rewrite mult_comm. reflexivity.
  Qed.
