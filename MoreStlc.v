...
Fixpoint subst (x:id) (s:tm) (t:tm) : tm :=
  ...
  | tunit =>
       tunit
  | tlet x' t1 t2 =>
       tlet x' (subst x s t1) (if (beq_id x x') then t2 else (subst x s t2) )
  | tinl T t1 =>
      tinl T (subst x s t1)
  | tinr T t1 =>
      tinr T (subst x s t1)
  | tcase t x1 t1 x2 t2 =>
      tcase (subst x s t) x1 (if beq_id x1 x then t1 else (subst x s t1)) x2  (if beq_id x2 x then t2 else (subst x s t2))
    (* lists *)
  | tnil T =>
      tnil T
  | tcons h t =>
       tcons (subst x s h) (subst x s t)
  | tlcase t1 t2 h t t3 =>
       tlcase (subst x s t1) (subst x s t2) h t (if (beq_id x h) then t3 else (if (beq_id x t) then t3 else (subst x s t3)))
  | tfix t1 =>
       tfix (subst x s t1)
  end.
...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_CaseInl: forall T v0 x1 t1 x2 t2,
        value v0 ->
        tcase (tinl T v0) x1 t1 x2 t2 ==> [x1:=v0] t1
  | ST_CaseInr: forall T v0 x1 t1 x2 t2,
        value v0 ->
  ...
  | ST_Lcase1: forall t1 t1' t2 t3 x y,
        t1 ==> t1' ->
        tlcase t1 t2 x y t3 ==> tlcase t1' t2 x y t3
  | ST_LcaseNil: forall T t2 x y t3,
       tlcase (tnil T) t2 x y t3 ==> t2
  | ST_LcaseCons: forall vh vt t2 x y t3,
       value vh ->
       value vt ->
       tlcase (tcons vh vt) t2 x y t3 ==>[x:=vh] ([y:=vt]t3)
  | ST_Fix1 : forall t1 t1',
       t1 ==> t1' ->
       tfix t1 ==> tfix t1'
  | ST_FixAbs: forall t1 xf T1 t2,
        t1 = tabs xf T1 t2 ->
        tfix t1 ==> [xf:=(tfix t1)] t2
where "t1 '==>' t2" := (step t1 t2).

Tactic Notation "step_cases" tactic(first) ident(c) :=
  first;
  [ Case_aux c "ST_AppAbs" | Case_aux c "ST_App1" | Case_aux c "ST_App2"
  | Case_aux c "ST_Succ" | Case_aux c "ST_Succ1" | Case_aux c "ST_Pred"
  | Case_aux c "ST_Pred1" | Case_aux c "ST_Pred2" | Case_aux c "ST_Mult" 
  | Case_aux c "ST_Mult1" | Case_aux c "ST_Mult2" | Case_aux c "ST_IfZero"   
  | Case_aux c "ST_IfNotZero" | Case_aux c "ST_If" | Case_aux c "ST_Pair1"
  | Case_aux c "ST_Pair2" | Case_aux c "ST_Fst1" | Case_aux c "ST_FstPair"
  | Case_aux c "Snd1" | Case_aux c "ST_SndPair" | Case_aux c "ST_Let1"
  | Case_aux c "ST_LetValue" | Case_aux c "ST_Inl" | Case_aux c "ST_Inr"
  ...
  | Case_aux c "ST_LcaseNil" | Case_aux c "ST_LcaseCons" | Case_aux c "ST_Fix1"
...
Inductive has_type : context -> tm -> ty -> Prop :=
  ...
  | T_Fst: forall t1 T11 T12 Gamma,
       Gamma |- t1 \in (TProd T11 T12) ->
       Gamma |- tfst t1 \in T11
  | T_Snd: forall t1 T11 T12 Gamma,
       Gamma |- t1 \in (TProd T11 T12) ->
  ...
  | T_Inr: forall Gamma t1 T1 T2,
        Gamma |- t1 \in T2 ->
        Gamma |- tinr T1 t1 \in (TSum T1 T2)
  | T_Case: forall Gamma x1 x2 t0 t1 t2 T T1 T2,
  ...
  | T_Cons : forall Gamma t1 T t2,
        Gamma |- t1 \in T ->
        Gamma |- t2 \in (TList T)->
        Gamma |- tcons t1 t2 \in (TList T)
  | T_Lcase : forall Gamma t1 T1 t2 h t t3 T,
...
Tactic Notation "has_type_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "T_Mult"
  | Case_aux c "T_If0" | Case_aux c "T_Pair" | Case_aux c "T_Fst"
  | Case_aux c "T_Snd" | Case_aux c "T_Unit" | Case_aux c "T_Let"
...
Proof. unfold test. 
  apply T_Let with (TArrow (TSum TNat TNat) TNat).
  apply T_Abs.
  apply T_Case with (T1:= TNat)(T2:= TNat).
  auto.
  auto.
  auto.
  auto. Qed.
...
Example typechecks :
  (@empty ty) |- test \in TNat.
Proof. unfold test. 
  apply T_Let with (TList TNat).
  apply T_Cons.
  auto.
  auto.
  apply T_Lcase with TNat .
  auto.
  auto.
  auto.  Qed.
...
Proof with eauto.
  ...
  Case "T_Succ".
    right. destruct IHHt...
    SCase "t1 is a value". 
      destruct H; (try inversion Ht); subst. exists (tnat (S n))...
    SCase "t1 can take a step".
      inversion H as [t1' Hstp]. exists (tsucc t1')...
  Case "T_Pred".
    right. destruct IHHt...
    SCase "t1 is a value". 
      destruct H; (try inversion Ht); subst.
      destruct n. exists (tnat 0)... exists (tnat n)...
    SCase "t1 can take a step".
      inversion H as [t1' Hstp]. exists (tpred t1')...
  Case "T_Mult".
    right. destruct IHHt1...
    SCase "t1 is a value".
      destruct H; (try inversion Ht1); subst.
      destruct IHHt2...
      SSCase "t2 is a value".
        destruct H; (try inversion Ht2); subst.
        exists (tnat (n*n0))...
      SSCase "t2 take a step".
        inversion H as [t2' Hstp]. exists (tmult (tnat n) t2')...
    SCase "t1 take a step".
        inversion H as [t1' Hstp]. exists (tmult t1' t2)...
  Case "T_If0".
    right. destruct IHHt1...
    SCase "t1 is value".
      destruct H; (try inversion Ht1); subst.
  ...
  Case "T_Pair".
    ...
    SCase "t1 is value".
      destruct IHHt2...
      right. inversion H0 as [t2' Hstp]. exists (tpair t1 t2')...
    SCase "t1 can take a step".
      right.
      inversion H as [t1' Hstp]. exists (tpair t1' t2)...
  Case "T_Fst".
    destruct IHHt...
    SCase "t1 is value".
      destruct H; try inversion Ht; subst.
  ...
  Case "T_Inl".
    destruct IHHt...
    inversion H as [t1' Hstp].
    right. exists (tinl T2 t1')...
  Case "T_Inr".
    destruct IHHt...
    inversion H as [t1' Hstp].
    right. exists (tinr T1 t1')...
  Case "T_Case".
    right.
    destruct IHHt1...
    destruct H; try inversion Ht1; subst.
  ...
  Case "T_Lcase".
    ...
    exists ([h:=v1]([t:=v2] t3))...
    inversion H as [t1' Hstp].
    exists (tlcase t1' t2 h t t3)...
  Case "T_Fix".
    right.
    destruct IHHt...
    destruct H; try inversion Ht; subst.
    exists ([x:=(tfix (tabs x T1 t12))] t12)...
    inversion H as [t1' Hstp]. exists (tfix t1')...
Qed.
...
Inductive appears_free_in : id -> tm -> Prop :=
  ...
  | afi_tsucc: forall x t1, 
      appears_free_in x t1->  
      appears_free_in x (tsucc t1)
  | afi_tpred: forall x t1, 
      appears_free_in x t1->  
  ...
  | afi_if03 : forall x t1 t2 t3,
      ...
      appears_free_in x (tif0 t1 t2 t3)
  | afi_tpair1: forall x t1 t2,
      appears_free_in x t1 ->
      appears_free_in x (tpair t1 t2)
  | afi_tpair2: forall x t1 t2,
      appears_free_in x t2 ->
      appears_free_in x (tpair t1 t2)
 | afi_tfst: forall x t1, 
      appears_free_in x t1->  
      appears_free_in x (tfst t1) 
 | afi_tsnd: forall x t1, 
      appears_free_in x t1->  
      appears_free_in x (tsnd t1) 
 | afi_tlet1: forall x y t1 t2,
      appears_free_in x t1 ->
      appears_free_in x (tlet y t1 t2 )
 | afi_tlet2: forall x y t1 t2,
      y <> x ->
      appears_free_in x t2 ->
      appears_free_in x (tlet y t1 t2 )
 | afi_tinl: forall t1 T x,
     appears_free_in x t1 ->
     appears_free_in x (tinl T t1)
 | afi_tinr: forall t1 T x,
     appears_free_in x t1 ->
     appears_free_in x (tinr T t1)
 | afi_tcase0: forall t x1 t1 x2 t2 x,
     appears_free_in x t ->
     appears_free_in x (tcase t x1 t1 x2 t2)
 | afi_tcase1: forall t x1 t1 x2 t2 x,
     x1 <> x ->
     appears_free_in x t1 ->
     appears_free_in x (tcase t x1 t1 x2 t2)
 | afi_tcase2: forall t x1 t1 x2 t2 x,
     x2 <> x ->
     appears_free_in x t2 ->
     appears_free_in x (tcase t x1 t1 x2 t2)
  | afi_tcons1: forall h t x,
      appears_free_in x h ->
      appears_free_in x (tcons h t)
  | afi_tcons2: forall h t x,
      appears_free_in x t ->
      appears_free_in x (tcons h t)
  | afi_tlcase0: forall t1 t2 h t t3 x,
      appears_free_in x t1 ->
      appears_free_in x (tlcase t1 t2 h t t3)
  | afi_tlcase1: forall t1 t2 h t t3 x,
      appears_free_in x t2 ->
      appears_free_in x (tlcase t1 t2 h t t3)
  | afi_tlcase2: forall t1 t2 h t t3 x,
      h <> x ->
      t <> x -> 
      appears_free_in x t3 ->
      appears_free_in x (tlcase t1 t2 h t t3)
  | afi_tfix: forall t1 x,
      appears_free_in x t1 ->
...
Proof with eauto.
  ...
  Case "T_Mult".
    apply T_Mult. 
    apply IHhas_type1. intros. apply Heqv. apply afi_tmult1. assumption.
    apply IHhas_type2. intros. apply Heqv. apply afi_tmult2. assumption.
  Case "T_If0".
    apply T_If0.
    apply IHhas_type1. intros. apply Heqv. apply afi_if01. assumption.
    apply IHhas_type2. intros. apply Heqv. apply afi_if02. assumption.
    apply IHhas_type3. intros. apply Heqv. apply afi_if03. assumption.
  Case "T_Pair".
    apply T_Pair.
    apply IHhas_type1. intros. apply Heqv. apply afi_tpair1. assumption.
    apply IHhas_type2. intros. apply Heqv. apply afi_tpair2. assumption.  
 Case "T_Let".
    apply T_Let with T1.
    apply IHhas_type1. intros. apply Heqv. apply afi_tlet1. assumption.  
    apply IHhas_type2. intros. unfold extend. remember (beq_id x x0) as e. 
    destruct e...
  Case "T_Case".
    apply T_Case with T1 T2.
    apply IHhas_type1. intros. apply Heqv. apply afi_tcase0. assumption.
    apply IHhas_type2. intros. unfold extend. remember (beq_id x1 x) as e. destruct e...
    apply IHhas_type3. intros. unfold extend. remember (beq_id x2 x) as e. destruct e... 
  Case "T_Cons".
     apply T_Cons.
     apply IHhas_type1. intros. apply Heqv. apply afi_tcons1. assumption.
     apply IHhas_type2. intros. apply Heqv. apply afi_tcons2. assumption.
  Case "T_Lcase".
     apply T_Lcase with T1.
     apply IHhas_type1. intros. apply Heqv. apply afi_tlcase0. assumption.
     apply IHhas_type2. intros. apply Heqv. apply afi_tlcase1. assumption.
     apply IHhas_type3. intros. unfold extend. 
     remember (beq_id t x) as et. 
     remember (beq_id h x) as eh.
     destruct et... destruct eh...
  Qed. 
...
Proof with eauto.
  ...
  Case "T_Lcase".
     ...
     apply not_eq_beq_id_false in H6. rewrite H6 in Hctx...
Qed.
...
Proof with eauto.
  ...
  Case "tlet".
   ...
   (* If [t = tlet y t1 t2], then we know that
         [Gamma,x:U |- tlet y t1 t2 : T2]
         [Gamma,x:U |- t1 : T1]
         [Gamma,x:U, y:T1 |- t2 : T2]
         [empty |- v : U]
       As our IH, we know that forall S Gamma, 
         [Gamma,x:U |- t1 : S -> Gamma |- [x:=v]t1 : S]
         [Gamma,x:U |- t2 : S -> Gamma |- [x:=v]t2 : S] .
    
       We can calculate that 
         [x:=v]t = tlet y ([x := v]t1) (if beq_id x y then t2 else [x := v]t2)
       And we must show that [Gamma |- [x:=v]t : T2].  We know
       We consider two cases: [x = y] and [x <> y].
       
       Case [x=y]. [x:=v]t= tlet y ([x := v]t1) t2.
       Then the thing to show is [Gamma |- tlet x [x:=v]t1 t2 : T2].
        By T_Let, it suffices to show that        
        [Gamma |- [x:=v] t1 \in T1] 
        [ extend Gamma x T1 |- t2 \in T2] 
       
        To show [Gamma |- [x:=v] t1 \in T1] , it suffices to show that [Gamma,x:U |- t1 : T1], 
        which is already in context.
        To show [ extend Gamma x T1 |- t2 \in T2] ., by context_invariance lemma and         
        [Gamma,x:U, x:T1 |- t2 : T2], it suffices to show that all free variables in t2 
        agree in context [extend Gamma x T1] and [Gamma,x:U, x:T1], which could be proved.
       
        Case [x <> y]. [x:=v]t = tlet [x:=v]t1 [x:=v]t2.
        Then the thing to show is [Gamma |- tlet y [x:=v]t1 [x:=v]t2 : T2].
        By T_Let. It suffice to show that [Gamma |- [x:=v]t1 : T1] and 
        [extend Gamma y T1 |- [x:=v]t2 : T2] .
        
         [Gamma,x:U,y:T1 |- t2 : T2]       =>
         [Gamma,y:T1,x:U |- t2 : T2]       =>
         [Gamma,y:T1 |- [x:=v]t2 : T2] *)
         remember (beq_id x y) as e. destruct e.
         apply T_Let with T1. 
             apply IHt1. assumption.
             apply beq_id_eq in Heqe. subst.
                 apply context_invariance with (Gamma:=(extend (extend Gamma y U) y T1)).
                 assumption. 
                 intros x Hafi. unfold extend. remember (beq_id x y) as e. destruct e... 
                   rewrite beq_id_sym in Heqe. rewrite <- Heqe. reflexivity.
                   rewrite beq_id_sym in Heqe. rewrite <- Heqe. reflexivity.
          apply T_Let with T1.
            apply IHt1. assumption.
            apply IHt2. apply context_invariance with (Gamma:=extend (extend Gamma x U) y T1).
              assumption. intros z Hafi. 
              unfold extend. 
              remember (beq_id x z) as e1. destruct e1; 
              remember (beq_id y z) as e2; destruct e2; try reflexivity. 
                apply beq_id_eq in Heqe1. apply beq_id_eq in Heqe2. subst. rewrite <- beq_id_refl in Heqe. inversion Heqe.
  Case "tcase".
        rename i into x1. rename i0 into x2. rename t1 into t0. rename t2 into t1. rename t3 into t2.
   (* If [t = tcase t0 x1 t1 x2 t2], then we know that
         [Gamma,x:U |- tcase t0 x1 t1 x2 t2 : T]
         [Gamma,x:U |- t0 : TSum T1 T2]
         [Gamma,x:U, x1:T1 |- t1 : T ]
         [Gamma,x:U, x2:T2 |- t2 : T ]
         [empty |- v : U]
       As our IH, we know that forall S Gamma, 
         [Gamma,x:U |- t1 : S -> Gamma |- [x:=v]t1 : S]
         [Gamma,x:U |- t2 : S -> Gamma |- [x:=v]t2 : S] .
       ...
       We can calculate that 
         [x:=v]t = tcase ([x := v]t0) x1 (if beq_id x1 x then t2 else [x := v]t2) x2
     (if beq_id x2 x then t3 else [x := v]t3)
       And we must show that [Gamma |- [x:=v]t : T].  We know
       We consider four cases: [x = x1] and [x <> x1], [x = x2] and [x <> x2].
       
       Case [x = x1] and [x = x2]. Then the thing to show is  [Gamma |- tcase [x:=v]t0 x1 t1 x2 t2: T].
       By T_Case, it suffices to show that 
       [Gamma|- [x:=v]t0 \in (TSum T1 T2)] (can be proved by IHt1)
       [extend Gamma x1 T1|-t1\in T] (can be proved by context_invariance lemma)
       [extend Gamma x2 T2|-t2\in T] (can be proved by context_invariance lemma)     

      Case [x = x1] and [x <> x2]. Then the thing to show is [Gamma |- tcase [x:=v]t0 x1 t1 x2 [x:=v]t2: T].
      By T_Case, it suffices to show that 
       [Gamma|- [x:=v]t0 \in (TSum T1 T2)] (can be proved by IHt1)
       [extend Gamma x1 T1|-t1\in T] (can be proved by context_invariance lemma)
       [extend Gamma x2 T2|-[x:=v]t2\in T] 
       To prove [extend Gamma x2 T2|-[x:=v]t2\in T], by IHt3, it suffices to show 
       [extend Gamma (extend Gamma x2 T2) x U |-t2\in T], which could be proved by context_invariance lemma.     
       
       Similar proof for the other two cases. *)
       remember (beq_id x1 x) as e1; destruct e1; 
       remember (beq_id x2 x) as e2; destruct e2.
       apply beq_id_eq in Heqe1; apply beq_id_eq in Heqe2. subst.
       apply T_Case with (T1:=T1)(T2:=T2).
         apply IHt1. assumption.
         apply context_invariance with (Gamma:=(extend (extend Gamma x U) x T1)). assumption.
           intros y Hafi. unfold extend. remember (beq_id x y) as e. destruct e...
         apply context_invariance with (Gamma:=(extend (extend Gamma x U) x T2)). assumption.
           intros y Hafi. unfold extend. remember (beq_id x y) as e. destruct e...
       ...
       apply T_Case with (T1:=T1)(T2:=T2).
         ...
         apply context_invariance with (Gamma:=(extend (extend Gamma x U) x T1)). assumption.
           intros y Hafi. unfold extend. remember (beq_id x y) as e. destruct e...
         apply IHt3. apply context_invariance with (Gamma:=(extend (extend Gamma x U) x2 T2)).
         assumption. intros y Hafi.
         unfold extend. remember (beq_id x y) as e1. destruct e1; 
         remember (beq_id x2 y) as e2; destruct e2; try reflexivity.
          apply beq_id_eq in Heqe1; apply beq_id_eq in Heqe0. subst. rewrite <- beq_id_refl in Heqe2. inversion Heqe2.
       apply beq_id_eq in Heqe2. subst.
       apply T_Case with (T1:=T1)(T2:=T2).
         apply IHt1. assumption.
         apply IHt2. apply context_invariance with (Gamma:=(extend (extend Gamma x U) x1 T1)).
         assumption. intros y Hafi.
         unfold extend. remember (beq_id x y) as e1. destruct e1; 
         remember (beq_id x1 y) as e2; destruct e2; try reflexivity.
          apply beq_id_eq in Heqe0; apply beq_id_eq in Heqe2. subst. rewrite <- beq_id_refl in Heqe1. inversion Heqe1.
         apply context_invariance with (Gamma:=(extend (extend Gamma x U) x T2)). assumption.
           intros y Hafi. unfold extend. remember (beq_id x y) as e. destruct e...     
       apply T_Case with (T1:=T1)(T2:=T2).
         apply IHt1. assumption.  
         ...
         unfold extend. remember (beq_id x y) as e1. destruct e1; 
         remember (beq_id x1 y) as e2; destruct e2; try reflexivity.
          apply beq_id_eq in Heqe0; apply beq_id_eq in Heqe3. subst. rewrite <- beq_id_refl in Heqe1. inversion Heqe1.
         apply IHt3. apply context_invariance with (Gamma:=(extend (extend Gamma x U) x2 T2)).
         assumption. intros y Hafi.
         unfold extend. remember (beq_id x y) as e1. destruct e1; 
         remember (beq_id x2 y) as e2; destruct e2; try reflexivity.
          apply beq_id_eq in Heqe0; apply beq_id_eq in Heqe3. subst. rewrite <- beq_id_refl in Heqe2. inversion Heqe2.

  Case "tlcase". 
    rename i into h. rename i0 into t. rename S into T.
   (* If [t = tlcase t1 t2 h t t3], then we know that
         [Gamma,x:U |- tlcase t1 t2 h t t3 : T]
         [Gamma,x:U |- t1 : TList T1]
         [Gamma,x:U |- t2 : T ]
         [Gamma,x:U, h:T1, t:(TList T1) |- t3 : T ]
         [empty |- v : U]
       As our IH, we know that forall S Gamma, 
         [Gamma,x:U |- t1 : S -> Gamma |- [x:=v]t1 : S]
         [Gamma,x:U |- t2 : S -> Gamma |- [x:=v]t2 : S] 
         [Gamma,x:U |- t3 : S -> Gamma |- [x:=v]t3 : S].
    
       We can calculate that 
         [x:=v]t = tlcase ([x := v]t1) ([x := v]t2) h t (if beq_id x h then t3 else if beq_id x t then t3 else [x := v]t3)
       And we must show that [Gamma |- [x:=v]t : T].  We know
       We consider four cases: [x = h] and [x <> h].
       
       Case [x = h]. Then the thing to show is  [Gamma |- tlcase ([x := v]t1) ([x := v]t2) h t t3: T].
       By T_Case, it suffices to show that 
         [Gamma |- [x:=v]t1 : TList T1] (by IHt1 )
         [Gamma |- [x:=v]t2 : T ](by IHt2)
         [Gamma, h:T1, t:(TList T1) |- t3 : T ] (by context_invariance lemma)

      Case [x<>h]. There are two subcases: [x = t] and [x <> t].
        SCase [x=t].Then the thing to show is  [Gamma |- tlcase ([x := v]t1) ([x := v]t2) h t t3: T]. Similar to the proof above.
        SCase [x<>t].Then the thing to show is  [Gamma |- tlcase ([x := v]t1) ([x := v]t2) h t [x:=v]t3: T].
          By T_Case, it suffices to show that 
             [Gamma |- [x:=v]t1 : TList T1] (by IHt1 and context_invariance lemma)
             [Gamma |- [x:=v]t2 : T ](by IHt2 and context_invariance lemma)
             [Gamma, h:T1, t:(TList T1) |- [x:=v]t3 : T ] (by IHt3 and context_invariance lemma)       
       *)
       remember (beq_id x h) as e1; destruct e1.
       apply T_Lcase with T1.
         apply IHt1. assumption.
         apply IHt2. assumption.
         apply beq_id_eq in Heqe1. subst. eapply context_invariance... 
            intros y Hafi. unfold extend. 
            remember (beq_id t y) as e1; destruct e1;
            remember (beq_id h y) as e1; destruct e1; try reflexivity.
      apply T_Lcase with T1.
         apply IHt1. eapply context_invariance...
         apply IHt2. eapply context_invariance...
          remember (beq_id x t) as e1; destruct e1.
          eapply context_invariance...
          intros y Hafi. unfold extend.
          remember (beq_id t y) as e1; destruct e1;
          remember (beq_id h y) as e1; destruct e1; try reflexivity.
          apply beq_id_eq in Heqe0. subst. rewrite <- Heqe2...
          apply IHt3. eapply context_invariance...
          intros y Hafi. unfold extend.
          remember (beq_id t y) as e1; destruct e1;
          remember (beq_id h y) as e1; destruct e1; try reflexivity.
           apply beq_id_eq in Heqe2. subst. rewrite <- Heqe0...
           apply beq_id_eq in Heqe2. subst. rewrite <- Heqe0...
           apply beq_id_eq in Heqe3. subst. rewrite <- Heqe1...
Qed.
