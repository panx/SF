...
(* When  a = X + 1, the assignement statement becomes {{ True }} X::= X+1 {{ X = X+1 }}, where Q is False.*)
(** [] *)
...
Proof.
  ...
  intros st st' HE HQ. 
  inversion HE. subst. inversion HQ.
   assert (Hst: st = (update (update st X (aeval st a)) X v)). apply functional_extensionality. intros x.
    rewrite update_shadow. rewrite <- H0. rewrite update_same. reflexivity. reflexivity. 
  split.
  rewrite <- Hst. assumption.
  rewrite update_eq. rewrite <- Hst. reflexivity.
  Qed.
  (* FILL IN HERE *) 
(** [] *)
...
Proof.
  ...
  apply hoare_asgn.
  intros st H. unfold assn_sub. unfold update. simpl. reflexivity.
Qed.
...
Example hoare_asgn_example':
  {{ fun st: state => st X + 1 <= 5 }}  X ::= APlus (AId X) (ANum 1)  {{ fun st: state => st X <= 5 }}.
Proof. eapply hoare_consequence_pre. apply hoare_asgn. unfold assert_implies. unfold assn_sub. intros st H. rewrite update_eq. apply H.
  Qed.
Example hoare_asgn_example'':
  {{ fun st: state => 0 <= 3 /\ 3 <= 5 }}  X ::= (ANum 3)  {{ fun st: state => 0 <=  st X /\ st X <= 5 }}.
Proof. eapply hoare_consequence_pre. apply hoare_asgn. unfold assert_implies. intros st H. unfold assn_sub. rewrite update_eq. simpl.  apply H.
  Qed.
(** [] *)
...
Proof.
  eapply hoare_seq. 
    ...
    eapply hoare_consequence_pre. 
      apply hoare_asgn. 
      intros st H. unfold assn_sub. simpl. unfold update. simpl. split. reflexivity. reflexivity. 
  Qed. 
...
Theorem swap_exercise :
  ...
  {{fun st => st Y <= st X}}.
Proof.
  unfold swap_program. eapply hoare_seq. eapply hoare_seq. apply hoare_asgn. apply hoare_asgn.  eapply hoare_consequence_pre. apply hoare_asgn.
...
Example hoarestate1: 
       forall (a : aexp) (n : nat),
         ...
         (X ::= (ANum 3); Y ::= a)
         {{fun st => st Y = n}}.
Proof. intros a n. eapply hoare_seq.  apply hoare_asgn. unfold assn_sub. unfold hoare_triple. intros st st' HE HQ. unfold update. simpl. 
  inversion HE. subst. destruct a. reflexivity. simpl. unfold update. 
(*Because a could be AId X.*)
Abort.
...
Proof.
  ...
  Case "Then". 
    ...
    simpl. intros st [_ H]. apply le_plus_minus. apply ble_nat_true in H. assumption.
  Case "Else". 
    eapply hoare_consequence_pre. apply hoare_asgn.
    unfold bassn, assert_implies, assn_sub, update.
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_If1True: forall (b1: bexp)(st st': state)(c1: com),
              beval st b1 = true -> c1/st||st' ->  (IF1 b1 THEN c1 FI)/st|| st'
  | E_If1False: forall (b1: bexp)(st: state)(c1: com),
...
Theorem hoare_if1 : forall P Q b c,
  ...
  {{P}} (IF1 b THEN c FI) {{Q}}.
Proof. intros P Q b c H1 H2 st st' H3 H4.
  inversion H3. subst.
  apply (H1 st st'). assumption. split. assumption. unfold bassn. assumption.
  subst.
  apply (H2 st'). apply E_Skip. split. assumption. unfold bassn, not. intros. rewrite H7 in H. inversion H.
  Qed.
...
Proof.
  ...
  Case "b=false".
    ...
    unfold bassn, assert_implies, assn_sub, update.
    intros st [H1 H2]. simpl in H2. unfold negb in H2. 
    destruct (st Y). simpl in H2. rewrite <- H1. omega.
...
Inductive ceval : state -> com -> state -> Prop :=
  ...
  | E_WhileLoop : forall st st' st'' b1 c1,
      ...
      ceval st' (WHILE b1 DO c1 END) st'' ->
      ceval st (WHILE b1 DO c1 END) st''
  | E_Repeat_Once: forall st st' b1 c1,
      ceval st c1 st' -> 
      beval st' b1 = true -> 
      ceval st (REPEAT c1 UNTIL b1 END) st'
  | E_Repeat_More: forall st st' st'' b1 c1,
      ceval st c1 st' ->
      beval st' b1 = false ->
      ceval st' (REPEAT c1 UNTIL b1 END) st'' ->
      ceval st (REPEAT c1 UNTIL b1 END) st''
...
Proof.
  unfold ex1_repeat. apply E_Repeat_Once. apply E_Seq with (st':= update empty_state X 1).
...
Theorem hoare_repeat: forall Q b c,
  {{fun st => True}} c {{Q}} ->{{fun st =>Q st /\ ~(bassn b st)}} c {{Q}} -> {{fun st: state=> True}} (REPEAT c UNTIL b END) {{fun st => Q st /\  (bassn b st)}}.
Proof. intros. unfold hoare_triple. intros. 
           ...
           split. apply (H st st'). assumption. assumption. apply bexp_eval_true. assumption.
...
In order to prove this, set P as {{ True }}, Q as {{Y>0}}, b as (X=0). First apply hoare_consequence_pre.
Then apply the above hoare_repeat theorem.
Then, we only needs to prove {{True}} c {{Q}}, and {{fun st =>Q st /\ ~(bassn b st)}} c {{Q}}.
*)
...
Definition havoc_pre (X : id) (Q : Assertion) : Assertion :=
  fun st: state => forall n, Q (update st X n). 

Theorem hoare_havoc : forall (Q : Assertion) (X : id),
  {{ havoc_pre X Q }} HAVOC X {{ Q }}.
Proof.
  intros. unfold hoare_triple. intros. unfold havoc_pre in H0. inversion H. subst. apply H0.
  Qed.
